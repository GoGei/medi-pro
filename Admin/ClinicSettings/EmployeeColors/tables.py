...
class EmployeeColorsTable(tables.Table):
    ...
    sideline = tables.TemplateColumn(
        template_code='<span class="badge" style="color: {{ record.sideline }}">{{ record.sideline }}</span>')
    background = tables.TemplateColumn(
        template_code='<span class="badge" style="color: {{ record.background }}">{{ record.background }}</span>')
    is_default = tables.BooleanColumn()
    is_active = tables.BooleanColumn()
    actions = custom_fields.DefaultActionFields(base_url='clinic-settings:employee-color')
